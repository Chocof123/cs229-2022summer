...
def initial_state(train_x):
    ...
    # *** START CODE HERE ***
    initial = {'train_x': train_x, 'beta': np.zeros(len(train_x))}
    return initial
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    beta = state['beta']
    K = np.empty(len(train_x)) #define kernel vector 
    for j, x_j in enumerate(train_x):
        K[j] = kernel(x_j,x_i)
    preds = sign(beta.dot(K))
    return preds
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, i, x_i, y_i):
    ...
    # *** START CODE HERE ***
    beta = state['beta']
    preds = predict(state,kernel,x_i) #predicted output 
    beta[i] += learning_rate * (y_i - preds) #update parameter 
    # *** END CODE HERE ***
