...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    transition_prob = mdp_data['transition_probs']
    V = mdp_data['value']
    #!!TBD: prob should be 163*163*2 matrix, but error reports (163,2) 
    #expect_val = transition_prob[state] @ V
    expect_val = V.T @ transition_prob[state]
    if expect_val[0] > expect_val[1]:
        return 0
    elif expect_val[0] == expect_val[1]:
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    transition_counts = mdp_data['transition_counts'] #num_states*num_states*num_actions
    reward_counts = mdp_data['reward_counts'] #num_states*2
    num_states = mdp_data['num_states'] 
    transition_probs = mdp_data['transition_probs'] #num_states*num_states*num_actions

    # C(old_state, action, new_state_0), C(old_state, action, new_state_1), ...
    # P(new_state_0 | old_state, action) + P(new_state_1 | old_state, action) ... = 1
    # P(new_state | old_state, action) = C(old_state, action, new_state) / SUM(C(old_state, action, *))
    
    #update transition probabilities 
    #!!TBD: why error warning of dividing by zero(total_prob_count)
    total_prob_counts = np.sum(transition_counts, axis=1)
    #try np.where to split up cases: 
    for old_state in range(num_states):
        for action in range(transition_counts.shape[2]):
            if total_prob_counts[old_state, action] != 0:
                transition_probs[old_state, :, action] = transition_counts[old_state, :, action] / total_prob_counts[old_state, action]
    #!!TBD: error reports unexpected keyword argument 'nan'
    #mdp_data['transition_probs'] = np.nan_to_num (transition_probs, copy=False, nan = 1/ num_states)
    # indx = np.where(np.isnan(transition_probs))
    # transition_probs[indx] = 1/ num_states
    mdp_data['transition_probs'] = transition_probs
    
    #update reward
    reward = - reward_counts[:,0] / reward_counts[:,1] #update reward as the avg reward in state s    
    #!!TBD: error reports unexpected keyword argument 'nan'
    #mdp_data['reward'] = np.nan_to_num (reward, copy=False, nan=0)
    indx = np.where(np.isnan(reward))
    reward[indx] = 0
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True: 
        #TBC!! CHECK LATER: Dim=0 choose max down the column, where each row represent one state/action  
        #value_update = reward + gamma * np.max(transition_probs.transpose(2,0,1) @ value, axis=0)
        value_update = reward + gamma * np.max(transition_probs.transpose(0,2,1) @ value, axis=1)
        result += 1
        val_diff = np.abs(value_update - value) 
        if np.max(val_diff) < tolerance: 
